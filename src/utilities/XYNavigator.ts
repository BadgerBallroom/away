 ...
 * ancestors," which are other DOM elements with a different class name. Locations are considered to share a horizontal
 * position if they are inside the same horizontal navigation ancestor. A location does NOT need to be a direct child of
 * its horitontal navigation ancestor, and horizontal navigation ancestors do NOT need to be siblings.
 *
...
export default class XYNavigator {
    ...
    public findLocation(
        ...
        const allDTCsInDestinationHNA = destinationHNA.getElementsByClassName(this.locationClassName);
        ...
        if (horizontalOffset === 0 && this._lastChosenX !== undefined) {
            ...
            const allDTCsInCurrentHNA = Array.from(
                currentHNA === destinationHNA
                    ? allDTCsInDestinationHNA
                    : currentHNA.getElementsByClassName(this.locationClassName)
            );

            x = allDTCsInCurrentHNA.indexOf(currentLocation);
        ...
        x = Math.min(Math.max(x, 0), allDTCsInDestinationHNA.length - 1);
        ...
        return allDTCsInDestinationHNA[x];
    }

    /**
     * Given a location, finds its horizontal navigation ancestor.
     * @param element The location
     * @returns The horitonzal navigation ancestor thaat was found (or `null` if it was not found)
     */
    private _findHorizontalNavigationAncestorOfLocation(element: Element | null): Element | null {
        while (element) {
            if (element.classList.contains(this.horizontalNavigationAncestorClassName)) {
                return element;
            }
    ...
    private _findAnotherHorizontalNavigationAncestor(verticalOffset: number, currentHNA: Element): Element {
        ...
        // Move up or down by moving from the current HNA to the previous or next HNA within the entire DOM.
