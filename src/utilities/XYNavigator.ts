 ...
 */
export default class XYNavigator {
     ...
     */
    private _findHorizontalNavigationAncestorOfLocation(element: Element | null): ElementHNA | null {
     ...
     */
    private _findOtherHorizontalNavigationAncestors(currentHNA: ElementHNA, verticalOffset: number): {
        precedingHNAs: ElementHNA[],
        followingHNAs: ElementHNA[],
    } {
        if (verticalOffset === 0) {
        ...
        const makeHNA = (element: Element) => new ElementHNA(element, this.locationClassName);
        return {
...
abstract class HorizontalNavigationAncestor {
    /** The original DOM element from which the locations came */
...
class ElementHNA extends HorizontalNavigationAncestor {
    ...
    public override getLocations(): Element[] {
        return Array.from(this.originalElement.getElementsByClassName(this._locationClassName));
    }
    ...
    public splitByWrappedLines(indexOfLocation: number): {
        ...
        verticallyGroupedLocations: VirtualHNA[],
        /**
 ...
 */
class VirtualHNA extends HorizontalNavigationAncestor {
    private _locations: Element[];

    constructor(originalElement: Element, locations: Element[]) {
        super(originalElement);
        this._locations = locations;
    }

    public override getLocations(): readonly Element[] {
        return this._locations;
    }
}

/** A helper class for navigating. */
class VerticalTraverser {
    public readonly lineWrapperClassName?: string;

    /** HNAs that come before the current HNA. */
    private _before: HorizontalNavigationAncestor[];
    /** The current HNA. */
    private _current: HorizontalNavigationAncestor;
    /** HNAs that come after the current HNA, in reverse order. */
    private _after: HorizontalNavigationAncestor[];
    ...
    public static create(
        ...
        before: readonly HorizontalNavigationAncestor[],
        current: HorizontalNavigationAncestor,
        trackIndex: number,
        after: readonly HorizontalNavigationAncestor[],
    ): {
        /** The instance of the helper class */
        verticalTraverser: VerticalTraverser,
        /** The new index of the location that was at {@link trackIndex} before (and if) {@link current} was split */
        xTracked: number,
    } {
        const verticalTraverser = new VerticalTraverser(lineWrapperClassName, before, current, after);
     ...
     */
    public moveBy(verticalOffset: number): HorizontalNavigationAncestor {
        if (verticalOffset < 0) {
            do {
                this.moveUpOnce();
                ++verticalOffset;
     ...
     * @returns The new current HNA
     */
    public moveUpOnce(): HorizontalNavigationAncestor {
     ...
     * @returns The new current HNA
     */
    public moveDownOnce(): HorizontalNavigationAncestor {
     ...
     */
    private _shouldSplitHNA(hna: HorizontalNavigationAncestor): hna is ElementHNA {
        return (
            !!this.lineWrapperClassName
