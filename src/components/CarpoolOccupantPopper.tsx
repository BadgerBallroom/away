...
export interface OccupantActionParameters {
     ...
     * @param isImmediatelyReopening Whether the popper is immediately showing again (default: `false`)
     */
...
const CarpoolOccupantPopper: React.FC<CarpoolOccupantPopperProps> = ({ action, setSnackbarProps }) => {
    ...
    useEffect(() => {
        ...
        if (previousAction.current !== action) {
            previousAction.current?.onClose(true, open);
...
function canSwapDancers(action: OccupantActionParameters | null): action is SwapDancersParameters {
    ...
    if (!action?.priorSelectedDancers.size) {
        return false;
    }
    ...
    for (const id of action.priorSelectedDancers) {
...
const SwapDancersButton: React.FC<SwapDancersButtonProps> = ({ action, activeDancerName, dancerMapState }) => {
    ...
    const isSwap = isDancerNotEmptySeat(action);
    const [priorSelectedCarpoolState, positionInPriorSelectedCarpool] = useMemo(() => {
        ...
        let carpoolStates = carpoolStatesOfDancers(action.priorSelectedDancers, action.carpoolArrangementState);
    ...
    const { icon, text } = useMemo(() => {
        const numPriorSelectedDancers = action.priorSelectedDancers.size;
        const priorSelectedDancerName = numPriorSelectedDancers === 1
            ? dancerMapState.getChildState(action.priorSelectedDancers.values().next().value!)?.getChildValue("name")
            : undefined;
        ...
        return { icon, text };
    }, [action, dancerMapState, isSwap, activeDancerName, intl]);
